...
const OthersProfile = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${apiUrl}/api/chat/ouser/data/${uid}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${access}`,
  ...
  useEffect(() => {
    const fetcwin_loss = async () => {
      try {
        const access = localStorage.getItem("access");
        const response = await axios.get(
          `${apiUrl}/api/chat/ouser/chart/${uid}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${access}`,
  ...
  const onmessagecklick = async () => {
    try {
      const access = localStorage.getItem("access");
      const response = await axios.get(
        `${apiUrl}/api/chat/ouser/getconv/${userData?.username}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${access}`,
      ...
      navigate(`/chat?convid=${response.data.id}`);
    } catch (error) {
      setErrors(errors);
  ...
  useEffect(() => {
    const fetchfriendreq = async () => {
      try {
        const access = localStorage.getItem("access");
        const response = await axios.get(
          `${apiUrl}/api/chat/ouser/friendreq/${uid}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${access}`,
  ...
  return (
    <div className="profile_user" key={userData?.username}>
