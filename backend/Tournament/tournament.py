...
class Tournament():
    ...
    async def add_player(self, user, obj):
        if self.users_num < 4:
            ...
            self.users_num = len(self.users)
            avatars_query = await sync_to_async(list)(
                User.objects.filter(id__in=self.users).values("id", "avatar")
            )

            # Step 1: Create a mapping of id to avatar
            avatars_map = {str(item["id"]): item["avatar"] for item in avatars_query}

            avatars = [
                f"/media/{avatars_map[user_id]}" 
                for user_id in self.users
                if avatars_map.get(str(user_id))
            ]
    ...
    def create_match(self, user1, user2): # here i create the room and add it to the pre_room_manager rooms dict
