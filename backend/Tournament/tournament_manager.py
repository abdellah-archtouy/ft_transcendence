...
class TournamentRoomManager:
    ...
    async def assign_user_intournament(self, name, obj):
        ...
        if tournament and not self.is_user_in_another_tournament(user):
            ...
            if added:
                tournament.group_name = f"tournament_info_{name}"  # i am using this group name to send updates for the users of tour
                obj.tournament = tournament
                await obj.channel_layer.group_add(
                    tournament.group_name, obj.channel_name
                )
                obj.joined = True
    ...
    async def monitor_tournament_status(self, obj, tournament):
        try:
            ...
            if tournament.winner:
                ...
                await obj.channel_layer.group_discard(
                    tournament.group_name, obj.channel_name
                )
    ...
    async def send_updates(self, obj, data):
        await obj.channel_layer.group_send(
            obj.group_name,
            {"type": "send_tournament", "tournament": {"tournaments_data": data}},
        )
    ...
    async def leave_tournament(self, obj):
        if obj.tournament:
            ...
            avatars_dict = await sync_to_async(list)(
                User.objects.filter(id__in=tournament.users).values("avatar")
            )
            avatars = [
                f"/media/{avatar['avatar']}"
                for avatar in avatars_dict
                if avatar["avatar"]
            ]
            ...
            if obj.channel_name:
                await obj.channel_layer.group_discard(
                    f"tournament_info_{obj.tournament.name}", obj.channel_name
                )
