...
def top5(request):
    try:
        ...
        my_sorted_list = [
            {**user, "link": f"/user/{user['id']}" if user["id"] != request.user.id else f"/profile"}
...
def assign_achievement(user):
    ...
    Criteria:
    - MAESTRO: 2 tournaments in a row as the winner.
    - DOWNKEEPER: 3 matches with no loser score.
    ...
    with transaction.atomic():
        ...
        if not user_achievement.downkeeper:
            ...
            ).order_by("-created_at")[:3]  # Fetch last 3 matches

            if (len(matches) == 3 and 
