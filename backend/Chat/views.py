...
def get_friend(request, username):
    try:
        user = request.user
        friend = User.objects.get(id=username)
...
def ConverstationView(request, convid):
    try:
        ...
        if conv.uid1 != user and conv.uid2 != user:
            return Response({"error": "Conversation not found"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({"error": "Conversation not found"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_messages(request, username):
    try:
        ...
        conv = Conversation.objects.filter(
            (Q(uid1=user.id) & Q(uid2=username)) | (Q(uid1=username) & Q(uid2=user.id))
        ).first()
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_ouser_data(request, uid):
    try:
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_ouser_win_and_lose(request, uid):
    try:
        now = timezone.now()
        start_of_today = now.replace(hour=0, minute=0, second=0, microsecond=0)
        user = User.objects.get(id=uid)
...
def get_mute_friend(request, username):
    try:
        user = request.user
        user2 = User.objects.get(id=username)
...
def create_Tournament_message(username, message):
    ...
    if user2 and tournement_user:
        ...
        link = f"/chat&convid={conversation.id}"
